...
class cadoo_invite implements api_interface
{
    public $guid;
    ...
    private function cadoo_invite()
    {
        try
        {
            $db = $this->_DB;

            $db->query(" SELECT param_value FROM messaging_operator_params WHERE ( operator = :operator AND param_key = 'invite_uri' ) ");
            $db->bind(':operator', $this->operator);
            $db->execute();
            $invite_uri = $db->fetchSingleColumn();

            if(empty($invite_uri))
                throw new Exception('No Send URI was found for this Organisation\'s operator.');

            $db->query(" SELECT param_value FROM messaging_operator_params WHERE ( operator = :operator AND param_key = 'invite_post_script' ) ");
            $db->bind(':operator', $this->operator);
            $db->execute();
            $invite_post_script = $db->fetchSingleColumn();

            if(empty($invite_post_script))
                throw new Exception('No Invite URI was found for this Organisation\'s operator.');

            $query_string = 'user=' . $this->operator_username . '&clipwd=' . $this->operator_password;
            $query_string.= $this->numbers_string;

            if($this->override)
                $query_string.= '&override=1';
            ...
            $socket = @fsockopen("$invite_uri", 80, $errno, $errstr, 120);
            if(!$socket)
                throw new Exception("Unable to get Neon server status.");

            $out = sprintf("POST $invite_post_script");
            $out.= " HTTP/1.1\n";
            ...
            foreach($results as $n => $res)
            {
                ...
                if(is_numeric($split[1]))
                {
                    ...
                    $status['successful'][$normalised_number] = $split[1];
                }
                else
    ...
    private function update_settings()
    {
        try
        {
            ...
            $nums = array_keys($this->response['successful']);
            ...
            if(empty($user_ids))
                throw new Exception('No User ID\'s were found for the supplied mobile numbers.');

            $db->query(" INSERT INTO user_setting 
              ( user_id, setting_key, setting_value, setting_year_starting, updated, updated_by ) VALUES 
              ( :user_id, 'cadoo_invited', 1, {$year_starting}, NOW(), {$sent_by} ) 
              ON DUPLICATE KEY UPDATE setting_value = (setting_value + 1), updated = NOW(), updated_by = {$sent_by} ");
