...
class pilot_member_update
{
    ...
    private function set_group_classes()
    {
        ...
        $db->query(" SELECT GroupID as id, GroupExtID as ext_id, GroupSuperGroupID as group_super_id 
        FROM `groups` 
        WHERE ( bool_GroupIsActive = 1 ) ");
        $db->execute();
    ...
    private function parse_members()
    {
        ...
        try
        {
            $db->query(" SELECT MAX(IndividualPaymentGroupID) FROM individuals WHERE 1 ");
            $db->execute();
            $pg_index = intval($db->fetchSingleColumn()) + 1;
            ...
            $db->query(" SELECT IndividualID as id 
            FROM individuals 
            WHERE ( IndividualExtID = :ext_id ) ");
            foreach($this->data['member'] as $m)
            {
                $m = array_change_key_case($m, CASE_LOWER);
                foreach ($m as $k => $v)
    ...
    private function update_members_in_group_classes()
    {
        ...
        try
        {
            ...
            $db->query(" SELECT IndividualID as id, IndividualGroupID as group_class_id 
            FROM individuals 
            WHERE (  
                IndividualID IN ({$member_ids_str})
            ) ");
            $current_group_per_member = $db->fetchAllAssoc('id', true);

            // determine which members have changed group
            ...
            foreach($this->member_id_ext_id_map as $id => $ext_id)
            {
                ...
                $members_in_new_groups[ $id ] = [
                    ...
                    'group_class_ext_id' => $this->group_classes_by_id[ $new_group_class_id ]['ext_id'],
            ...
            {
                $db->query(" UPDATE individuals SET 
                IndividualGroupID = :group_class_id, IndividualGroupExtID = :group_class_ext_id 
                WHERE ( IndividualID = :id ) ");
                foreach($members_in_new_groups as $id => $grp)
                {
                    $db->bind(':id', $id);
                    $db->bind(':group_class_id', $grp['group_class_id']);
                    $db->bind(':group_class_ext_id', $grp['group_class_ext_id']);
                    $db->execute();
                }
            }
    ...
    private function insert_new_members($new_members)
    {
        ...
        try
        {
            ...
            $db->query(" INSERT INTO individuals 
                    ( IndividualExtID, IndividualFName, IndividualLName, 
                     IndividualPaymentGroupID, IndividualGroupID, IndividualFutureGroupID, IndividualGroupExtID, 
                     IndividualStartDate, IndividualEndDate, bool_IndividualIsActive, 
                     IndividualCreated, IndividualModified ) VALUES 
                    ( :IndividualExtID, :IndividualFName, :IndividualLName, 
                     IndividualPaymentGroupID, IndividualGroupID, IndividualFutureGroupID, IndividualGroupExtID, 
                     IndividualStartDate, IndividualEndDate, bool_IndividualIsActive, 
                     IndividualCreated, IndividualModified )");
