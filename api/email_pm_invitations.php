...
try
{
    ...
    foreach($tmp as $i => $t)
    {
        $dates[] = DateTime::createFromFormat('d-m-Y', $t['date'])->format('l jS \of F');
        $day_details[ $t['id'] ] = [
            'date' => $t['date'],
            'venue' => $t['venue'],
    ...
    $dates_str = implode(', and ',$dates);

    $status[] = 'Event Days set';


    $org_db->query(" SELECT DISTINCT p_gua.email
    FROM person p
    LEFT JOIN member_guardian mg ON ( p.id = mg.id AND is_default = 1 AND guardian_end IS NULL )
    LEFT JOIN person p_gua ON ( mg.guardian_id = p_gua.id )
    WHERE
    ( 
        p.is_member = 1 AND p.active = 1 AND p.id NOT IN
        (
          SELECT DISTINCT member_id FROM nsadmin_pm_reservation WHERE ( event_id = :event_id AND is_break = 0 )
        )
    ) ");
    $org_db->bind(':event_id', $event_id);
    $org_db->execute();
    ...
    #$emails = ['eamonn@databizsolutions.ie'];
    ...
    $root_path = JCT_PATH_SECTIONS . 'assets' . JCT_DE . 'templates' . JCT_DE . 'email' . JCT_DE;
    $html = file_get_contents($root_path . 'html' . JCT_DE . 'en_default.html');


    $body_html = <<<EOS
<p style="margin: 0 0 0.5rem">Dear Parent / Guardian,</p>
<p style="margin: 0 0 0.5rem; font-weight: bold">An upcoming Parent Meeting ($event_title) will be held on $dates_str.</p>
...
We ask that you make use of this facility (instructions below) at the earliest opportunity in order to reserve a convenient time-slot for your own meeting.</p>
...
<ol style="margin: 0 0 1rem">
    ...
    <li>Choose the 'Schedule your Parent Meetings' option</li>
...
EOS;
    ...
    $html = strtr($html,
        [
            ...
            '$_TITLE'=>'Parent / Teacher Meetings',
