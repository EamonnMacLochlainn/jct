...
try
{
    ...
    foreach($raw_reservations as $res_id => $r)
    {
        ...
        $org_db->query(" SELECT email FROM person WHERE ( id IN ({$guardian_ids_str}) AND email IS NOT NULL ) ");
        $org_db->execute();
        $guardian_emails = $org_db->fetchAllColumn();

        foreach($guardian_emails as $email)
        {
...
EOS;
    ...
    $html = strtr($html,
        [
            ...
            '$_TITLE'=>'FÓGRA / Notification of meeting',
            '$_FOOTER'=>$html_footer
        ]
    );
