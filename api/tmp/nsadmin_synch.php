...
try
{
    ...
    $post = file_get_contents('sample_data/11111A.json',false, $context);
    ...
    try
    {
        foreach($persons as $i => $person)
        {
            ...
            try
            {
                ...
                ( ext_id, user_key, session_id, email, mobile, pass, updated, updated_by ) VALUES 
                ( :ext_id, :user_key, NULL, :email, :mobile, NULL, NOW(), -1 ) 
            ...
            catch(Exception $e)
            {
                ...
                if($code == 23000)
                {
                    ...
                    $db->query(" INSERT INTO user 
                        ( user_key, session_id, email, mobile, pass, updated, updated_by ) VALUES 
                        ( :user_key, NULL, :email, :mobile, NULL, NOW(), -1 ) 
                        ON DUPLICATE KEY UPDATE 
    ...
    try
    {
        ...
        $org_db->query(" SELECT tbl_id, id, group_staff_id FROM group_staff_staff WHERE in_group_end IS NULL ");
        $org_db->execute();
        $tmp = $org_db->fetchAllAssoc();
        ...
        $org_db->query(" UPDATE group_staff_staff SET in_group_end = DATE(NOW()), updated = NOW(), updated_by = -1 WHERE in_group_end IS NULL ");
        $org_db->execute();
        ...
        if(!empty($concurrent_records))
        {
            ...
            $org_db->query(" UPDATE group_staff_staff SET in_group_end = NULL WHERE tbl_id IN ({$concurrent_records_str}) ");
            $org_db->execute();
        }

        if(!empty($new_records))
        {
            $org_db->query(" INSERT INTO group_staff_staff 
        ...
        $org_db->query(" UPDATE group_super SET active = 0, active_end = NOW(), updated = NOW(), updated_by = -1 WHERE 1 ");
        $org_db->execute();
        ...
        $org_db->query(" INSERT INTO group_super 
                ( ext_id, title, title_eng_gae_variant, abbr, active, active_begin, active_end, 
                  academic_order, academic_band, 
                  current, weight, updated, updated_by ) VALUES 
                ( :ext_id, :title, NULL, :abbr, :active, NOW(), NULL, 0, 0, :current_, 0, NOW(), -1 ) 
                ON DUPLICATE KEY UPDATE 
                id = last_insert_id(id), ext_id = :ext_id, title = :title, abbr = :abbr, 
                active = :active, active_end = :active_end, current = :current_, updated = NOW(), updated_by = -1 " );
        ...
        foreach($data['group_super'] as $group_super)
        {
            ...
            $active_end = ($active) ? NULL : $upload_datetime->format('Y-m-d');
            ...
            $org_db->bind(':abbr', $abbr);
            $org_db->bind(':active', $active);
            $org_db->bind(':active_end', $active_end);
        ...
        $org_db->query(" UPDATE group_class SET active = 0, active_end = NOW(), updated = NOW(), updated_by = -1 WHERE 1 ");
        $org_db->execute();
        ...
        $org_db->query(" INSERT INTO group_class 
                ( ext_id, title, title_eng_gae_variant, abbr, group_super_id, gender, active, active_begin, active_end, weight, updated, updated_by ) VALUES 
                ( :ext_id, :title, NULL, :abbr, :group_super_id, :gender, :active, NOW(), NULL, 0, NOW(), -1 ) 
                ON DUPLICATE KEY UPDATE 
                id = last_insert_id(id), ext_id = :ext_id, title = :title, abbr = :abbr, 
                group_super_id = :group_super_id, gender = :gender, active = :active, active_end = :active_end, updated = NOW(), updated_by = -1 " );
        ...
        $org_db->query(" SELECT tbl_id, id, group_class_id FROM group_class_leader WHERE leader_end IS NULL ");
        $org_db->execute();
        $tmp = $org_db->fetchAllAssoc();
        ...
        $org_db->query(" UPDATE group_class_leader SET leader_end = DATE(NOW()), updated = NOW(), updated_by = -1 WHERE leader_end IS NULL ");
        $org_db->execute();
        ...
        {
            $concurrent_records_str = implode(',',$concurrent_records);
            $org_db->query(" UPDATE group_class_leader SET leader_end = NULL WHERE tbl_id IN ({$concurrent_records_str}) ");
            $org_db->execute();
        }

        if(!empty($leader_to_groups_map))
        {
            $org_db->query(" INSERT INTO group_class_leader 
        ...
        $org_db->query(" SELECT tbl_id, id, group_assistant_id FROM member_group_assistant WHERE assistant_end IS NULL ");
        $org_db->execute();
        $tmp = $org_db->fetchAllAssoc();
        ...
        $org_db->query(" UPDATE member_group_assistant SET assistant_end = DATE(NOW()), updated = NOW(), updated_by = -1 WHERE assistant_end IS NULL ");
        $org_db->execute();
        ...
        {
            $concurrent_records_str = implode(',',$concurrent_records);
            $org_db->query(" UPDATE member_group_assistant SET assistant_end = NULL WHERE tbl_id IN ({$concurrent_records_str}) ");
            $org_db->execute();
        }

        if(!empty($new_records))
        {
            $org_db->query(" INSERT INTO member_group_assistant 
...
catch (Exception $e)
{
    ...
    $status[] = \JCT\Helper::show($last_record, true);
