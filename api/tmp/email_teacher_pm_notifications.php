...
try
{
    ...
    $staff_id_causing_replacement = 685;
    $day_id_being_replaced = 3;
    $replacement_date = '28-11-2018';
    ...
    $org_db->bind(':event_id', $event_id);
    $org_db->bind(':day_id', $day_id_being_replaced);
    $org_db->execute();
    ...
    #\DS\Helper::show($guardian_reservations);
    #exit();
...
<p style="margin: 0 0 1rem;">Due to unforeseen circumstances, it was necessary to re-schedule your Parent Meeting from its original date. It will now be held, 
at the original time, on $replacement_date. Please see below for exact details.</p>
<p style="margin: 0;">The following [RESERVATIONS_STR] been made for you by School administrators:</p>
<ul>[RESERVATIONS]</ul>
<p style="text-align: center; font-weight: bold; margin-bottom:2rem;">Go raibh míle maith agat.</p>
EOS;
